 ...
 <div class="box">
    ...
    <div class="box-body">
		<div class="pull-right">
		    <form class="form-inline" action="<?php echo base_url('admin/users/teachers/id/desc') ?>">
		      <div class="box-body">
		        ...
		        <a style="margin-top:0px;" href="<?php echo base_url('admin/users/teachers/id/desc');  ?>"  style="margin-top:3px;"class="btn btn-warning">Reset</a>
	    ...
	    <thead>
			<tr>
			<th width="5%">#</th>
			<th width="15%"><small>Name of teacher</small></th>
			...
			$i = 0; foreach ($teachers as $row) { $i++;
				...
				<tr>
					...
					<td >
						...
						<div class="btn-group ">
					<button type="button" class="btn <?php if($row->status==1): ?>
					btn-success <?php elseif($row->status==2): ?> btn-danger <?php else: ?>  btn-warning  <?php endif; ?> dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					<?php if($row->status==1): ?> Active <?php elseif($row->status==2): ?> Deactive <?php else: ?> Pending <?php endif; ?> <span class="caret"></span>
					</button>
					<ul class="dropdown-menu">
					<li><a href="<?php echo base_url('admin/users/updateStatus/' . $row->id ) ?>/1/teachers">Active</a></li>
					<li><a href="<?php echo base_url('admin/users/updateStatus/' . $row->id ) ?>/2/teachers">Deactive</a></li>		
					</ul>
					</div>
					...
					<td>
					<a href="<?php echo base_url('admin/users/teachers/view/' . $row->id ) ?>" class="btn btn-success btn-xs" rel="tooltip" data-placement="left" data-original-title=" View ">
							<i class="fa fa-eye"></i>
						</a>
					
					<a href="<?php echo base_url('admin/users/teachers/edit/' . $row->id ) ?>" class="btn btn-success btn-xs" rel="tooltip" data-placement="left" data-original-title=" Edit ">
							<i class="fa fa-pencil"></i>
						</a>
						<a href="<?php echo base_url('admin/users/teachers/delete/' . $row->id) ?>" class="btn btn-danger btn-xs" rel="tooltip" rel="tooltip" data-placement="bottom" data-original-title="Remove" onclick="return confirm('Are you sure want to delete?');" >
					...
					<tr>
						<th colspan="7">
							<center>No teachers Found.</center>
						</th>
					</tr>
				<?php endif; ?>
			</tbody>
