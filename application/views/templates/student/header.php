<?php 
    ...
    if($segment2 == 'Search' || $segment3 == 'Search/index'){  $search='active'; }else{ $search = '';  }
...
<style type="text/css">
.log_error{color:red;}.ui-datepicker td{padding: 1px !important;}
.loader_place {
    display:    none;
    position:   fixed;
    z-index:    9999;
...
<body class="<?php  if(!empty($feed)){ echo 'classfeed';  }else if(!empty($task_chapter)){ echo 'chapter'; }else if(!empty($profile)){ echo 'classfeed profiles'; }else if(!empty($search)){ echo 'search'; }else if(!empty($note)){ echo 'my-notes'; }else if(!empty($task)){ echo 'task'; }else if(!empty($library)){ echo 'library'; } ?>">
  ...
  <div id="loader" class="loader_place"><!-- Place at bottom of page --></div>
    <div  id="wrapper">
    ...
    <li>
            ...
            <!-- end navbar-header -->
                ...
                <ul class="nav navbar-top-links navbar-left">
                    <li><input type="text" name="search" value="<?php  if(isset($_POST['search']) && !empty($_POST['search'])){ echo  $_POST['search'];   }?>" placeholder="Search"><button   type="submit" name="search_submit"><i class="material-icons icon-align black">search</i></button></li><!--  -->
                </ul>
            </form>
